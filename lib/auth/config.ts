// OAuth configuration for production
const clientMetadata = {
  client_id: 'https://atproto.at/oauth-client-metadata.json',
  client_name: 'Taproot (atproto.at://) - AT Protocol Explorer',
  client_uri: 'https://atproto.at',
  logo_uri: 'https://atproto.at/logo.png',
  tos_uri: 'https://atproto.at/terms',
  policy_uri: 'https://atproto.at/privacy',
  redirect_uris: [
    'https://atproto.at/auth/callback',
    'https://atproto.at/auth'
  ],
  scope: 'atproto transition:generic',
  grant_types: ['authorization_code', 'refresh_token'],
  response_types: ['code'],
  token_endpoint_auth_method: 'none',
  application_type: 'web',
  dpop_bound_access_tokens: true
};

// OAuth configuration
export const AUTH_CONFIG = {
  clientId: process.env.NEXT_PUBLIC_CLIENT_ID || clientMetadata.client_id,
  redirectUri: process.env.NEXT_PUBLIC_REDIRECT_URI || 'https://atproto.at/auth',
  scope: 'atproto transition:generic',
  responseType: 'code' as const,
  grantType: 'authorization_code' as const,
  clientMetadata: clientMetadata
};

// Helper to get the correct redirect URI based on environment
export function getRedirectUri() {
  if (typeof window !== 'undefined') {
    const origin = window.location.origin;
    return `${origin}/auth/callback`;
  }
  return AUTH_CONFIG.redirectUri;
}